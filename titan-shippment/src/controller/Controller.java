...
public class Controller implements ActionListener {
	...
	public void trigger(String action, int x, int y) {
		if (action.equals("click_map")) {
			...
			// clic sur la carte aux coordonn�es (x,y) and get the associated point (if any)
	...
	public void trigger(String action, String name) {
		if (addingNewLivraison) {
			interfaceView.displayAlert("Impossible d'effectuer l'action demand�e", "Vous ne pouvez pas charger de fichier ou calculer une nouvelle tourn�e pendant l'ajout d'un point de livraison.", "warning");
		}
		else {
			if (action.equals("loadFile")) {
				if (name.equals("loadMap")) {
					...
					if (filename != null && filename.length() > 0) {
						...
						if (buildOk) {
							...
							interfaceView.displayAlert("Erreur au chargement de la carte", "La carte n'a pas �t� charg�e correctement.", "error");
					...
					if (!mapLoaded) {
						interfaceView.displayAlert("Impossible de charger les livraisons", "Vous devez charger une carte au pr�alable.", "warning");
					}
					else {
						String filename = interfaceView.loadFile();
						...
						if (filename != null && filename.length() > 0) {
							...
							if (buildOk) {
								...
								interfaceView.displayAlert("Erreur au chargement des livraisons", "Les livraisons n'ont pas �t� charg�es correctement", "error");
								interfaceView.repaint();
								return;
							}
				...
				if (name.equals("calculTournee")) {
					if (!mapLoaded || !livraisonsLoaded) {
						interfaceView.displayAlert("Impossible de calculer la tourn�e", "Vous devez charger une carte et une livraison au pr�alable.", "warning");
					}
					else {
						resetTournee();
					...
					if(!undoRedo.Undo()) {
						interfaceView.displayAlert("UNDO", "Rien � annuler", "info");
					}
				}
				else if (name.equals("redo")) {
					if(!undoRedo.Redo()) {
						interfaceView.displayAlert("REDO", "Rien � r�tablir", "info");
					}
				}
